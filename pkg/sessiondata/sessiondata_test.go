...
func TestNewSessionData(t *testing.T) {
	...
	session := NewSessionData(req, bodyBytes, headers, nil, HTTP11Protocol)
	...
	if session.Protocol != HTTP11Protocol {
		t.Errorf("Expected protocol %s, got %s", HTTP11Protocol, session.Protocol)
	}
}
...
func TestRequestDifferences(t *testing.T) {
	session1 := createTestSession("GET", "https://example.com/api", "original body",
		map[string]interface{}{"Authorization": "Bearer token1"},
		map[string]string{"session": "abc123", "user": "john"})

	session2 := createTestSession("POST", "https://example.com/api", "modified body",
		map[string]interface{}{"Authorization": "Bearer token2"},
		map[string]string{"session": "xyz789", "role": "admin"})
...
func TestToCurlEscaping(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			session := createTestSession(method, "https://api.example.com/test", tt.body, tt.headers, map[string]string{})
			result := session.ToCurl()
...
func createTestSession(method, urlStr, body string, headers map[string]interface{}, cookies map[string]string) *Session {
	return &Session{
		ID:        "test-id",
		Timestamp: time.Now(),
		Request: &RequestData{
...
func BenchmarkNewSessionData(b *testing.B) {
	...
	b.ResetTimer()
	for i := 0; i < b.N; i++ {
		NewSessionData(req, bodyBytes, headers, nil, HTTP11Protocol)
	}
}
...
func BenchmarkCompareRequest(b *testing.B) {
	...
	session2 := createTestSession("POST", "https://example.com/api", "different body",
		...
		map[string]string{"session": "xyz789"})
...
func BenchmarkRequestDifferences(b *testing.B) {
	...
	session2 := createTestSession("POST", "https://example.com/api/v2", "modified body",
		map[string]interface{}{"Authorization": "Bearer token2"},
		map[string]string{"session": "xyz789", "role": "admin"})

	b.ResetTimer()
	for i := 0; i < b.N; i++ {
		session1.RequestDifferences(session2)
	}
}
